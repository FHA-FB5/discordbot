...
async function commandHandler(message: Message, client: Client) {
  ...
  if (command.cooldown) {
    ...
    if (cooldownTimestamp) {
      ...
      if (now < cooldownExpirationTime) {
        message.reply(new InfoMessageEmbed({ description: getMessage('command.cooldown', { count: Math.floor((cooldownExpirationTime - now) / 1000), locale: options.locale }) }));
  ...
  try {
    ...
    message.reply(new ErrorMessageEmbed({ description: getMessage('error.unknown', { locale: options.locale }) }));
  }
}
